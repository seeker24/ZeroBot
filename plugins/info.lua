do
local mohammadsz = 257261175
local erfan = 224661370
local Clever = 235384878
...
local function res_user_callback(extra, success, result) -- /info <username> function
  ...
  if result.username then
   ...
   Username = '----'
    ...
    if not value then
	 if result.peer_id == tonumber(mohammadsz) then
	   text = text..'》Rank : Executive Admin \n\n'
	   elseif result.peer_id == tonumber(erfan) then
	   text = text..'》Rank : 👤 SUDO \n\n'
  ...
  local text = '》Full name : '..(result.from.first_name or '')..' '..(result.from.last_name or '')..'\n'
	...
	local hash = 'rank:'..result.to.id..':variables'
		local value = redis:hget(hash, result.from.peer_id)
		 if not value then
		    if result.from.peer_id == tonumber(mohammadsz) then
		       text = text..'》Rank : Executive Admin \n\n'
			   elseif result.peer_id == tonumber(erfan) then
	           text = text..'》Rank : 👤 SUDO \n\n'
  ...
  if msg.from.username then
   ...
   local hash = 'rank:'..msg.to.id..':variables'
	if hash then
	  ...
	  if not value then
		if msg.from.id == tonumber(mohammadsz) then
		 text = text..'》Rank : Executive Admin \n\n'
		 elseif msg.from.id == tonumber(erfan) then
		 text = text..'》Rank : 👤 SUDO \n\n'
